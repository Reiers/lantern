...
// EC finality assumption which is based on worst-case conditions.
//
// Why Lantern wants this: F3 gives BLS-verified finality certificates, but
// where F3 finality is not available at the tip (or not live on a network),
// the fallback is the static 900-epoch worst case. FRC-0089 gives a
// principled, locally-computed finality depth with no external trust: it
// quantifies how deep a tip-range eclipse could plausibly reorg us. It
// feeds the head-trust story (#100), retention pruning depth (#92), and
// the v2 "finalized"/"safe" selector semantics (#99:
// finalized = max(ec-finalized, F3-finalized)).
//
// This is a port of lotus chain/ecfinality (itself a Go port of the
// FRC-0089 Python reference), adapted to Lantern's header store. The math
// is kept verbatim so the lotus/Python test vectors apply unchanged.
//
...
const (
	// BisectLow and BisectHigh define the search range for the bisect
	// algorithm that finds the epoch depth at which the finality guarantee
	// is met. A low bound of 3 avoids evaluating trivially shallow depths;
	// a high bound of 450 accommodates degraded chains that take longer to
	// finalize.
	BisectLow  = 3
	BisectHigh = 450

	// DefaultBlocksPerEpoch is the Filecoin expected block production rate
	// (mainnet and calibration both target 5).
	DefaultBlocksPerEpoch = 5.0

	// DefaultByzantineFraction is the standard Filecoin security assumption
	// for adversarial mining power.
...
// tipset could be reorganized out of the canonical chain. Verbatim port of
// the FRC-0089 reference (finality_calc_validator.py) via lotus.
//
// Parameters:
//   - chain: block counts per epoch (index 0 = earliest epoch; null rounds
//     appear as 0 entries)
...
// FindThresholdDepth performs a bisect search to find the shallowest depth
// at which the reorg probability drops below the given guarantee. Returns
// -1 if the guarantee is not met within the search range.
